...
export class TemplateManager {
  ...
  async createFromTemplate(templateName: string, data: TemplateData): Promise<void> {
    const templateContent = this.getTemplateContent(templateName, data);
    
    await fs.writeFile('asset-manifest.toml', templateContent, 'utf-8');
    
    console.log(`✅ Created manifest from template: ${templateName}`);
    console.log(`📋 Generated: asset-manifest.toml`);
  }

  private getTemplateContent(templateName: string, data: TemplateData): string {
  ...
  private getReactAppTemplate = (data: TemplateData): string => `# ${data.projectName} Asset Manifest
...
`;
  ...
  private getNextJsAppTemplate = (data: TemplateData): string => `# ${data.projectName} Asset Manifest
...
`;
  ...
  private getMarketingSiteTemplate = (data: TemplateData): string => `# ${data.projectName} Marketing Site
...
`;
  ...
  private getMobileAppTemplate = (data: TemplateData): string => `# ${data.projectName} Mobile App
...
`;
  ...
  private getDashboardTemplate = (data: TemplateData): string => `# ${data.projectName} Dashboard
...
`;
  ...
  private getEcommerceTemplate = (data: TemplateData): string => `# ${data.projectName} E-commerce
